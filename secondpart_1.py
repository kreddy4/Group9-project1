        ...
        for term in terminals:
            if(term not in st):
                ...
                if(st[term] == '1'):
                    ...
                    break
        # check how many 1's we counted
        if count % 2 == 0:  # if more than one 1, we know it's going to be 0.
            circuit[node][3] = '1'
        else:  # Otherwise, the output is equal to how many 1's there are
            circuit[node][3] = '0'
        return circuit
...
def basic_sim(circuit,st={}):
    ...
    while True:
        ...
        if term_has_value:
            circuit[curr][2] = True
            circuit = gateCalc(circuit, curr,st)
...
def main():
    ...
    while True:
        inputName1 = "input.txt"
        print("\n Read input vector file: use " + inputName1 + "?" + " Enter to accept or type filename: ")
        userInput = input()
        if userInput == "":

            break
        else:
            inputName1 = os.path.join(script_dir, userInput)
            if not os.path.isfile(inputName1):
                print("File does not exist. \n")
            else:
                break

    while True:
        inputName2 = "f_list.txt"
        print("\n Read input vector file: use " + inputName2 + "?" + " Enter to accept or type filename: ")
        userInput = input()
        if userInput == "":

            break
        else:
            inputName2 = os.path.join(script_dir, userInput)
            if not os.path.isfile(inputName2):
                print("File does not exist. \n")
            else:
                break
    ...
    print("\n *** Simulating the" + inputName1 +" "+inputName2+ " file and will output in" + outputName + "*** \n")
    ...
    for line in inputFile1:
        ...
        o="tv = "+line+"-> " + output + "(good)\n"
        outputFile.write(o)

        print("----------------------")


        print("if there is any fault file............")
        inputFile2 = open(inputName2, "r")
        ...
        for line1 in inputFile2:
            ...
            output3=output2
            st={}
            #creating the dictionary for fault stucks

            nc=copy.deepcopy(circuit)
            #deepcopying the list of lists of circuit
            ...
            cou=line1.count("-")#for differentiating the faults
            ...
            for y1 in nc["OUTPUTS"][1]:
                if not nc[y1][2]:
                    output = "NETLIST ERROR: OUTPUT LINE \"" + y1 + "\" NOT ACCESSED"
                    break
                output3 = str(nc[y1][3])+output3
